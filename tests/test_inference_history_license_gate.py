...
class InferenceHistoryLicenseGateTests(unittest.TestCase):
    ...
    def test_list_inference_jobs_does_not_require_inference_license(self):
        with patch.object(app.ProjectManager, "get_project", return_value=self.project), patch.object(
            app.InferenceHistory, "list_jobs", return_value={"jobs": []}
        ), patch.object(app, "require_feature", side_effect=AssertionError("history read should not require feature gate")):
            result = app.list_inference_jobs("proj_history_gate")
    ...
    def test_get_inference_job_does_not_require_inference_license(self):
        ...
        with patch.object(app.ProjectManager, "get_project", return_value=self.project), patch.object(
            app.InferenceHistory, "get_job", return_value=payload
        ), patch.object(app, "require_feature", side_effect=AssertionError("history read should not require feature gate")):
            result = app.get_inference_job("proj_history_gate", "job_1")
    ...
    def test_run_image_inference_still_requires_inference_license(self):
        ...
        with patch.object(app, "require_feature", side_effect=blocked_feature):
            with self.assertRaises(HTTPException) as ctx:
                asyncio.run(app.run_image_inference("proj_history_gate"))
