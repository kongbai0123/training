...
class UIHardeningPhasesStaticTests(unittest.TestCase):
    ...
    def test_phase4_informational_training_flow_is_connected_to_both_overviews(self):
        ...
        self.assertIn("renderTrainingFlowGuide", guide_js)
        self.assertIn("const CNN_PHASES", guide_js)
        self.assertIn("const RNN_PHASES", guide_js)
        self.assertIn('trainingFlow.cnn.stage.annotation', guide_js)
        self.assertIn('trainingFlow.rnn.stage.window', guide_js)
        self.assertIn('trainingFlow.preview.cnn.title', guide_js)
        self.assertIn('trainingFlow.preview.rnn.title', guide_js)
        self.assertIn('data-ui-smoke="training-flow-guide"', guide_js)
        self.assertNotIn("is-complete", guide_js)
        self.assertNotIn("is-current", guide_js)
        self.assertNotIn("progress", guide_js.lower())
        self.assertIn('renderTrainingFlowGuide({ mode: appState.currentProject ? "cnn" : null })', dashboard_js)
