...
| Optional Hugging Face CLI extras | `typer` imported only by `huggingface_hub.inference._mcp` CLI modules | Ignore | Vision Training Studio does not expose the Hugging Face MCP command-line interface; packaged model download and inference paths do not import it. |
